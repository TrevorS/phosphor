...
///
/// The verbs are hidden from the top-level help rather than absent from it: 216
...
/// not listed, which is why 216 capabilities need no table here. The single
...
mod tests {
    ...
    fn the_host_flags_survive_the_extension() {
        // One parser, not two: adding 216 subcommands must not cost `--theme`

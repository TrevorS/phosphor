...
///
/// The verbs are hidden from the top-level help rather than absent from it: 208
...
/// not listed, which is why 208 capabilities need no table here. The single
...
mod tests {
    ...
    fn the_host_flags_survive_the_extension() {
        // One parser, not two: adding 208 subcommands must not cost `--theme`

...
RECORDED = {
    ...
    "ExpandDiffContext": ("", "No creditor — checked against every mockup this task drew and "
                             ...
                             "does not either, and `T070`'s design-brief line names only the "
                             "mode, not a context control. Recorded rather than guessed at a "
                             "task, the same shape `revert-hunk` takes below."),
    "RevertHunk": ("", "**No creditor — checked against every mockup that mentions reverting, "
                       "and none draws this verb.** OPEN-QUESTIONS.md §59 ruled the before-side "
                       "question `revert-hunk` was recorded against and `T066` built it — a "
                       "review block now carries what claude's edit replaced, on `FileGroup`'s "
                       "own `ChangedSpan`. That made the *rich* revert buildable, and reading the "
                       "mockups against it is what closed this row instead: `TUI Mockups.dc.html`'s "
                       "`6d` draws the only \"revert\" key any screen has — "
                       "\"dih  delete inner hunk — revert claude's edit, plain vim delete\" — "
                       "which is `T026`'s delete operator over `T064`'s hunk text object, already "
                       "built and reachable. `4b`'s own footer has no revert key at all, and `2b`'s "
                       "`u undo (jj)` is `T073`'s different verb over a different store. "
                       "`a_declared_hunks_dih_reverts_it_plain_vim_delete_style` proves `dih` "

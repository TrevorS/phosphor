...
**And one more in the repair window after `CP-4`**, which makes `6b` the screen with four
amendments against it. This one is not a drawing losing to a running program — it is a ruling
about what the product *should* do, so it is the first amendment here that a test could not have
found.

| | amends | what changes |
|---|---|---|
| `CP-4` | TUI Mockups `6b` | **A bare `(keymap-set! …)` does not persist; `(persist! …)` does.** `6b`'s fourth line answers `⇒ #ok · persisted to init.scm` for a form nobody asked to keep — persistence by head name, which means trying a theme keeps it forever. Ruled by Teej on 2026-08-14 against Emacs, which has two mechanisms and neither is this one: `M-:` and `ielm` never persist, and `M-x customize` is a deliberate *save this* that writes `custom-file`. It also fails the third invariant, *nothing moves unless you asked*. The mechanism is kept and the automatic is gone: `persist!` is an identity function in `runtime/repl.scm`, so the REPL stays the only writer and a persisted form is idempotent at boot; a bare config verb now answers `· not persisted — (persist! …) keeps it`, which is `6b`'s own receipt offering the verb. `7a`'s always-allow is untouched — pressing a digit was already explicit. Built as `T101`; the reasoning and the still-open half are at [OPEN-QUESTIONS.md](OPEN-QUESTIONS.md)'s §32. |
| `CP-4` | — | **The persisted file leaves the tree**, and this amends no drawing — it is [Q1](#q1) applied a second time. `phosphor/persist-file` was a bare name joined to the runtime root, and in a dev checkout that root is the repository: `CP-4`'s own manual test left a `(define-language! "lua" …)` in a tracked `runtime/persisted.scm`. It is `$XDG_CONFIG_HOME/phosphor/persisted.scm` now — **config, not state**, because a binding you kept belongs with your dotfiles and nothing about a keymap is per-project, which is exactly the property that makes Q1's root hashing right for undo and wrong here. `crates/phosphor-core/src/config.rs` is `journal.rs`'s sibling and follows it. |

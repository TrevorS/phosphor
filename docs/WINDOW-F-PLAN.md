...
**Verification:** `just tapes-diff` must be clean on `1a`, `1a-seeded`, `3c`, `8e`, `2a`, `6d` — the whole screen is drawn by a different mechanism and must come out identical. Re-capture `1a-degraded-no-colour` specifically for the `fill` fix. `just review` on the golden frames. `scripts/lint-node-kinds.sh` via `just lint`.
...
- **Verification:** `just gate` (all six, CI's order) after each of 14 commits · `just tapes-diff` on `1a`, `1a-seeded`, `1a-degraded-no-colour`, `3c`, `8e`, `2a`, `6d` at steps 2, 3 and 11, with `git checkout -- tapes` after each · `just review` on the golden frames · new unit tests: `PaneTree` split/focus/close/resize/resolve, `PaneTree::layout` at 1/2/3 panes and odd widths, per-buffer `edits`/`sent`, keyed `Outstanding`, selector routing (`SetCursor`, reveal, mouse hit-test), swap-clears-`selection_from` · CP-6's manual half
